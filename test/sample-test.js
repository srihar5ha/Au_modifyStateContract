...
describe("TestModifyVariable", function () {
  it("should change x to 1337 and myname to monkeydluffy", async function () {
    ...
    const contract = await ModifyVariable.deploy(10,"zoroman");
    ...
    const newName=await contract.myname();
    console.log("new name is ",newName);
    assert.equal(newX.toNumber(), 1337);
    assert.equal(newName.toString(), "monkeydluffy");
  });
});